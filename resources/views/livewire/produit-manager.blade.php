    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 dark:bg-gray-800">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-100">Gestion des Produits</h2>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    ...
                                    @if($produit->code_barre)
                                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $produit->code_barre }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $produit->nom }}</div>
                                    <div class="text-sm text-gray-500 dark:text-gray-400">{{ Str::limit($produit->description, 30) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    {{ number_format($produit->prix_vente, 2) }} Fc
                                    <div class="text-xs text-gray-500 dark:text-gray-400">{{ number_format($produit->prix_achat, 2) }} Fc (achat)</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                        {{ $produit->stock > $produit->seuil_alerte ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' : 
                                          ($produit->stock > 0 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100' : 'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100') }}">
                                        {{ $produit->stock }} {{ $produit->unite_mesure }}
                                    </span>
                                    @if($produit->stock <= $produit->seuil_alerte)
                                        <div class="text-xs text-red-500 dark:text-red-400">Seuil: {{ $produit->seuil_alerte }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                                    @if($produit->sousRayon)
                                        {{ $produit->sousRayon->rayon->nom }} / {{ $produit->sousRayon->nom }}
                                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $produit->sousRayon->code_emplacement }}</div>
                                    @else
                                        Non assigné
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <button wire:click="addStock({{ $produit->id }})" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300" title="Ajouter du stock">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                        </button>
                                        <button wire:click="edit({{ $produit->id }})" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300" title="Modifier">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                            </svg>
                                        </button>
                                        <button wire:click="delete({{ $produit->id }})" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300" title="Supprimer" onclick="confirm('Êtes-vous sûr de vouloir supprimer ce produit ?') || event.stopImmediatePropagation()">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <form wire:submit.prevent="{{ $action == 'create' ? 'store' : 'update' }}">
                            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 dark:bg-gray-800">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div class="mb-4">
                                        <label for="seuil_alerte" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Seuil d'alerte:</label>
                                        <input type="number" id="seuil_alerte" wire:model.defer="seuil_alerte" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:focus:ring-indigo-500">
                                        @error('seuil_alerte') <span class="text-red-500 text-xs dark:text-red-400">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="mb-4">
                                        <label for="unite_mesure" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Unité de mesure:</label>
                                        <input type="text" id="unite_mesure" wire:model.defer="unite_mesure" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:focus:ring-indigo-500">
                                        @error('unite_mesure') <span class="text-red-500 text-xs dark:text-red-400">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="mb-4 flex items-center">
                                        <input type="checkbox" id="taxable" wire:model.defer="taxable" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-indigo-500">
                                        <label for="taxable" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">Taxable</label>
                                    </div>
                                </div>
                                <div class="mb-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="mb-4">
                                        <label for="rayon_id" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Rayon:</label>
                                        <select id="rayon_id" wire:model="rayon_id" wire:change="$refresh" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:focus:ring-indigo-500">
                                            <option value="">Sélectionner un rayon</option>
                                            @foreach($rayons as $rayon)
                                                <option value="{{ $rayon->id }}">{{ $rayon->nom }}</option>
                                            @endforeach
                                        </select>
                                        @error('rayon_id') <span class="text-red-500 text-xs dark:text-red-400">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="mb-4">
                                        <label for="sous_rayon_id" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Sous-rayon:</label>
                                        <select id="sous_rayon_id" wire:model="sous_rayon_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:focus:ring-indigo-500" {{ !$rayon_id ? 'disabled' : '' }}>
                                        ...
                                        @error('sous_rayon_id') <span class="text-red-500 text-xs dark:text-red-400">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="mb-4">
                                        <label for="reference_interne" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Référence interne:</label>
                                        <input type="text" id="reference_interne" wire:model.defer="reference_interne" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:focus:ring-indigo-500">
                                        @error('reference_interne') <span class="text-red-500 text-xs dark:text-red-400">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="mb-4">
                                        <label for="code_barre" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Code-barres:</label>
                                        <input type="text" id="code_barre" wire:model.defer="code_barre" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300 dark:focus:ring-indigo-500">
                                        @error('code_barre') <span class="text-red-500 text-xs dark:text-red-400">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                            </div>
                            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse dark:bg-gray-800">
